...
{	uint32 AlgorValue,AlogorHex;
	uint8 HexByte,i;
	for(i=0,AlgorValue=1,AlogorHex=0;i<SizeofBCD;i++,AlgorValue=AlgorValue*100)
	{	Bcd2Hex(BCD+SizeofBCD-i-1,&HexByte,1);	
		AlogorHex=AlogorHex+HexByte*AlgorValue;
	}
	return AlogorHex;
}
...
{	uint32 AlgorValue;
	uint8 i,Byte;
	for(i=0,AlgorValue=100000000;i<5;i++)
	{	Byte=Hex/AlgorValue;
		Hex2Bcd(&Byte,BCD+i,1);
		Hex=Hex%AlgorValue;
		AlgorValue=AlgorValue/100;		
	}
}
