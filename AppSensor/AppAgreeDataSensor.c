...
{	uint16 i;
	...
	for(i=0;i<SizeofParID;i++,WatchDog())
	...
	CN_st_Fun_RTC.Read(BackDateTimer);
}														
...
{	uint16 i;
	...
	for(i=0;i<sizeof(CN_Ba_WordPar)/sizeof(uint16);i++,WatchDog())
...
{	uint32 *Vol,AddTim; 
	...
	CN_st_Fun_AscNumber.MoveByte(BackDateTimer,DateTime,sizeof(BackDateTimer));
	JugeVo(AddTim,*Vol);					
}
...
{	st_Parameter_Table	CN_st_Parameter_Table;	
	...
	if(!((CN_st_Parameter_Table.CN_st_Sum_Waring.Voltage.WartingVl==0xff*1000)&&
	(CN_st_Parameter_Table.CN_st_Sum_Waring.Voltage.WartingTimer==0xff)))
	{	if(CN_st_Parameter_Table.CN_st_Sum_Waring.Voltage.WartingVl>=Speed)
		{	AddTimeHigh+=AddTim;
			...
			{	if(CN_st_Fun_TakeData.GetState(De_Access_Space_LowVo)!=De_Waring_State_Yes)
				{	App_CN_st_WorkFlow_Table.CN_st_AgreeOn_Send_Basic.State[De_Orientation_State_Low]=
...
{	uint8 *Val;
	st_Parameter_Table	CN_st_Parameter_Table;
	Val=Data;	
	if((*Val!=De_Sensor_ACC_Open)&&(App_CN_st_WorkFlow_Table.CN_st_AgreeOn_Send_Basic.State[De_Orientation_State_Help]!=De_AgreeOn_State_Erro)) 	
		return ;
	CN_st_Fun_OpenAndClose.OpenGps();
	CN_st_Fun_OpenAndClose.OpenGprs();
	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.Parameterfd,0,&CN_st_Parameter_Table);
