...
{	if(AppSendAgreeOnSem!=NULL)
	...
	if((CN_Box_AppSend=OSMboxCreate (NULL))==NULL)
		return De_Erro_AppSendAgree_Init;
	if((AppSendAgreeOnSem1=OSSemCreate(0))==NULL)
		return De_Erro_AppSendAgree_Init;
...
{	st_AppSendAgreeOn_Cm	CN_st_AppSendAgreeOn_Cm;
	uint8 err;
	...
	CN_st_Fun_AscNumber.ClsArray(&CN_st_AppSendAgreeOn_Cm,sizeof(st_AppSendAgreeOn_Cm));
	CN_st_AppSendAgreeOn_Cm.Command=De_SendCm_NoSemSend;
	OSMboxPost(CN_Box_AppSend,&CN_st_AppSendAgreeOn_Cm);
	OSSemPend(AppSendAgreeOnSem1,0, &err);
	if(CN_st_AppSendAgreeOn_Cm.Result!=De_Suc_AppSendAgree)
		SendLogin();		
	OSSemPost(AppSendAgreeOnSem);
}
static uint16 WriteFlow(uint8 ID,st_WorkFlow_Table	*ptst_WorkFlow_Table,uint8 VidType)	 	//写流水表
{	st_AppSendAgreeOn_Cm_WriteFlow	CN_st_AppSendAgreeOn_Cm_WriteFlow;
	st_AppSendAgreeOn_Cm	CN_st_AppSendAgreeOn_Cm;
	uint8 err;
	OSSemPend(AppSendAgreeOnSem,0, &err);
	CN_Pu_st_st_AgreeOn_Dev_WritePar.OrderNumber=0;
	...
	CN_Pu_st_st_AgreeOn_Dev_WritePar.ContrID=0;
	CN_st_Fun_AscNumber.ClsArray(&CN_st_AppSendAgreeOn_Cm,sizeof(st_AppSendAgreeOn_Cm));
	CN_st_AppSendAgreeOn_Cm.Command=De_SendCm_WriteFlow;
	CN_st_AppSendAgreeOn_Cm.Buf=&CN_st_AppSendAgreeOn_Cm_WriteFlow;
	CN_st_AppSendAgreeOn_Cm_WriteFlow.ID=ID;
	CN_st_AppSendAgreeOn_Cm_WriteFlow.ptst_WorkFlow_Table=ptst_WorkFlow_Table;
	CN_st_AppSendAgreeOn_Cm_WriteFlow.VidType=VidType;
	OSMboxPost(CN_Box_AppSend,&CN_st_AppSendAgreeOn_Cm);
	OSSemPend(AppSendAgreeOnSem1,0, &err);
	if(CN_st_AppSendAgreeOn_Cm.Result!=De_Suc_AppSendAgree)
		SendLogin();
	OSSemPost(AppSendAgreeOnSem);
	return CN_st_AppSendAgreeOn_Cm.Result;
}

static uint16 WriteCommand(uint8 fd,uint16 Par,st_st_AgreeOn_Dev_Par	*ptst_st_AgreeOn_Dev_Par) 									//发送命令
{	st_AppSendAgreeOn_Cm_WriteCommand	CN_st_AppSendAgreeOn_Cm_WriteCommand;
	st_AppSendAgreeOn_Cm	CN_st_AppSendAgreeOn_Cm;
	uint8 err;
	OSSemPend(AppSendAgreeOnSem,0, &err);
	CN_Pu_st_st_AgreeOn_Dev_WritePar.OrderNumber=0;
	...
	CN_st_Fun_AscNumber.ClsArray(&CN_st_AppSendAgreeOn_Cm,sizeof(st_AppSendAgreeOn_Cm));
	CN_st_AppSendAgreeOn_Cm.Command=De_SendCm_WriteCommand;
	CN_st_AppSendAgreeOn_Cm.Buf=&CN_st_AppSendAgreeOn_Cm_WriteCommand;
	CN_st_AppSendAgreeOn_Cm_WriteCommand.fd=fd;
	CN_st_AppSendAgreeOn_Cm_WriteCommand.ptst_st_AgreeOn_Dev_Par=ptst_st_AgreeOn_Dev_Par;
	CN_st_AppSendAgreeOn_Cm_WriteCommand.Par=Par;
	OSMboxPost(CN_Box_AppSend,&CN_st_AppSendAgreeOn_Cm);
	OSSemPend(AppSendAgreeOnSem1,0, &err);
   	if(CN_st_AppSendAgreeOn_Cm.Result!=De_Suc_AppSendAgree)
		SendLogin();
...
{	st_AppSendAgreeOn_Cm	*ptst_AppSendAgreeOn_Cm;
	st_AppSendAgreeOn_Cm_WriteFlow	*ptst_AppSendAgreeOn_Cm_WriteFlow;
	st_AppSendAgreeOn_Cm_WriteCommand	*ptst_AppSendAgreeOn_Cm_WriteCommand;
	uint8 err;
	for(;;)
	{	ptst_AppSendAgreeOn_Cm=OSMboxPend (CN_Box_AppSend,0,&err);
		if(ptst_AppSendAgreeOn_Cm->Command==De_SendCm_NoSemSend)				//盲区补偿
		{	ptst_AppSendAgreeOn_Cm->Result=NoSemSend1();
		}
		else if(ptst_AppSendAgreeOn_Cm->Command==De_SendCm_WriteFlow)
		{	ptst_AppSendAgreeOn_Cm_WriteFlow=ptst_AppSendAgreeOn_Cm->Buf;
			ptst_AppSendAgreeOn_Cm->Result=WriteFlow2(ptst_AppSendAgreeOn_Cm_WriteFlow->ID,
			ptst_AppSendAgreeOn_Cm_WriteFlow->ptst_WorkFlow_Table,ptst_AppSendAgreeOn_Cm_WriteFlow->VidType);
		}
		else if(ptst_AppSendAgreeOn_Cm->Command==De_SendCm_WriteCommand)
		{	ptst_AppSendAgreeOn_Cm_WriteCommand=ptst_AppSendAgreeOn_Cm->Buf;
			ptst_AppSendAgreeOn_Cm->Result=Send(ptst_AppSendAgreeOn_Cm_WriteCommand->fd,
			ptst_AppSendAgreeOn_Cm_WriteCommand->Par,
			ptst_AppSendAgreeOn_Cm_WriteCommand->ptst_st_AgreeOn_Dev_Par->Buf);
		}
		OSSemPost(AppSendAgreeOnSem1);
		WatchDog();
	}	
}
...
{	st_AgreeOn_Send_LogAndNoSem	*ptst_AgreeOn_Send_LogAndNoSem;
	...
	CN_st_Fun_AscNumber.MoveByte(&ptst_AgreeOn_Send_LogAndNoSem->CN_Ba_st_AgreeOn_Send_Basic,
...
{	st_Parameter_Table	CN_st_Parameter_Table;
	...
	if(CN_st_Parameter_Table.CN_st_Data_Access.ScotomaLog[0]==0)
	...
	for(Number=i,ptst_AgreeOn_Send_LogAndNoSem->Number=0,DataLen=0;i<=Cuit;i++,WatchDog())
...
{	st_Parameter_Table	CN_st_Parameter_Table;
	...
	uint8 fd,Oper;
	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.Parameterfd,0,&CN_st_Parameter_Table);
	if(CN_st_Parameter_Table.CN_st_Data_Access.ScotomaLog[1]==0)
	...
	for(Number=i,DataLen=0,ptst_AgreeOn_Send_LogAndNoSem->Number=0;i<=Cuit;i++,WatchDog())
	{	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.NoFlowfd,i,&CN_st_NoFlow_Table);
...
{	st_Parameter_Table	CN_st_Parameter_Table;
	...
	uint8 fd,Oper;
	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.Parameterfd,0,&CN_st_Parameter_Table);
	if(CN_st_Parameter_Table.CN_st_Data_Access.ScotomaLog[3]==0)
...
{ 	st_Parameter_Table	CN_st_Parameter_Table;
	...
	uint8 fd,Oper;
	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.Parameterfd,0,&CN_st_Parameter_Table);
	if(CN_st_Parameter_Table.CN_st_Data_Access.ScotomaLog[2]==0)
	{	CN_st_Fun_TakeData.AddSpace(De_ReadSpace_Reduce,De_Space_Set);
	   	return De_Suc_AppSendAgree;
	}
	Cuit=CN_st_Fun_TakeData.ReadSpace(De_ReadSpace_Reduce);
	...
	ptst_AgreeOn_Send_LogAndNoSem->Number=0;
	for(Number=i;i<=Cuit;i++,WatchDog())
	{	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.NoFlowfd,i,&CN_st_NoFlow_Table);
...
{	st_Parameter_Table	CN_st_Parameter_Table;
	...
	ptst_AgreeOn_Send_LogAndNoSem->Number=0;
	for(Number=i,DataLen=0;i<=Cuit;i++,WatchDog())
	{	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.NoFlowfd,i,&CN_st_NoFlow_Table);
...
{	uint8 i;
	...
	CN_Pu_st_st_AgreeOn_Dev_WritePar.OrderNumber=0;
	for(i=0;i<sizeof(CN_Ba_st_Par2Fun_Par)/sizeof(st_Par2Fun_Par);i++,WatchDog())
...
{	st_WorkFlow_Table	CN_st_WorkFlow_Table;
	...
	{	ptst_AgreeOn_Send_Waring->CN_st_WorkVal_Tabel.Type[i]=De_st_AgreeOn_Send_Waring_Type_No;
		if(CN_st_Fun_TakeData.GetState(De_Access_Space_WorkParWaring1+i)==De_Waring_State_Yes)
			ptst_AgreeOn_Send_Waring->CN_st_WorkVal_Tabel.Type[i]=De_st_AgreeOn_Send_Waring_Type_Yes;	
	}
...
{	union
	...
	uint8 Oper,Type;
	Recoder=Buf;
	ptst_AgreeOn_Send_UpLoaderDriver=CN_Pu_st_st_AgreeOn_Dev_WritePar.Buf;
...
{	st_AgreeOn_Send_UpLoaderHandMess	*ptst_AgreeOn_Send_UpLoaderHandMess;
	...
	ptst_UpLoaderHandMess_Input->ID,sizeof(ptst_UpLoaderHandMess_Input->ID));
	
	CN_st_Fun_AscNumber.MoveByte(ptst_AgreeOn_Send_UpLoaderHandMess->Phone,
	ptst_UpLoaderHandMess_Input->Phone,sizeof(ptst_UpLoaderHandMess_Input->Phone));
	
	CN_st_Fun_AscNumber.MoveByte(ptst_AgreeOn_Send_UpLoaderHandMess->Cut,
	ptst_UpLoaderHandMess_Input->Cut,sizeof(ptst_UpLoaderHandMess_Input->Cut));
	return CN_st_Fun_Tzv21.Write(fd,&CN_Pu_st_st_AgreeOn_Dev_WritePar,&Oper);	
}
...
{	st_Reduce_Table	CN_st_Reduce_Table;
	...
	CN_st_Fun_Tz_Send.Write(Data,&CN_st_Reduce_Table,ReLen,De_Dev_Order_UpLoaderReduce);
	return De_Suc_AppSendAgree;				
}
