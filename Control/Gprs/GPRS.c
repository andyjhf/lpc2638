...
{	uint16 Result;	
	uint8 err;
	OSSemPend(GprsUseSem, 0, &err);
	Result=GetIpPro(fd,ptst_Data_UDPTCP);
 	OSSemPost(GprsUseSem);
	return Result;
}
...
static uint16 GetCurIPState(uint8 fd,uint8 *State)
{	uint16 Result;	
	uint8 err;
	OSSemPend(GprsUseSem, 0, &err);
	Result=CN_st_Fun_GprsHard.Read(De_GprsHard_Com_SEEIPCLOSE,(void *)&CN_LinkNumber[fd],NULL,State,NULL);	
 	OSSemPost(GprsUseSem);
	return Result;	
}
							  		 													//GSM网络信息
...
{	uint8 *UnMessage=CN_IPData;
	uint8 *CentrPhone=(uint8 *)(CN_IPData+512);
	uint32 SendSize;
	uint16 Result,ReSize;
	CN_st_Fun_AscNumber.ClsArray(CentrPhone,15);
	if((Result=CN_st_Fun_GprsHard.Read(De_GprsHard_Com_GetCenterNumber,NULL,NULL,CentrPhone,&ReSize))!=De_Suc_GprsHard)
		return Result;
	CN_st_Fun_AscNumber.ClsArray(UnMessage,512);
...
static uint16 GetPhoneState(st_Data_Gprs_Call	*ptst_Data_Gprs_Call1)
{	if(CN_st_Affair_Gprs.PhoneState!=De_Gprs_State_Open)
		return De_Erro_Gprs_FunctionNo;
	CN_st_Fun_AscNumber.MoveByte(ptst_Data_Gprs_Call1,ptst_Data_Gprs_Call,sizeof(st_Data_Gprs_Call));
	return De_Suc_Gprs;
}
static uint16 GetIpPro(uint8 fd,st_Data_UDPTCP	*ptst_Data_UDPTCP)
{	CN_st_Fun_AscNumber.MoveByte(ptst_Data_UDPTCP,&ptst_Data_Gprs_TcpUdp[fd]->CN_st_Data_UDPTCP,sizeof(st_Data_UDPTCP));		
	return De_Suc_Gprs;
...
{	uint8 err;					
	pData=pData;
	for(;;OSSemPost(GprsUseSem),OSTimeDly(De_Gprs_TaskDly),WatchDog())
	{	OSSemPend(GprsUseSem, 0, &err);
		if(CN_st_Affair_Gprs.SysState!=De_Gprs_State_Open)					//系统电源没有开
			continue;
//		if(CN_st_Affair_Gprs.ModelState==De_Gprs_ModelState_Erro)
//		{	
//		}
...
{	uint8 DateTimer1[7];	
	...
	uint32 TimerDly1;
	uint16 Result,i;
	uint8 fd,err,j,LinkState;
	CN_st_Fun_RTC.Read(DateTimer1);
	TimerDly=GetTimeAdd(DateTimer,DateTimer1);
	CN_st_Fun_AscNumber.MoveByte(DateTimer,DateTimer1,7);
	for(i=0;i<De_GprsHard_MaxLink;i++)
...
//		ptst_Data_Gprs_TcpUdp[i]->CN_st_Heart_Timer.CentrHeartTimer+=TimerDly;
	...
	{	if(ptst_Data_Gprs_TcpUdp[j]->OpenOrClose==De_GprsHard_Link_Close)
			continue;
		if(ptst_Data_Gprs_TcpUdp[j]->CN_st_Data_UDPTCP.CentrHeartTimer==0)
			continue;
		CN_st_Fun_AscNumber.MoveByte(&TimerDly1,&ptst_Data_Gprs_TcpUdp[j]->CN_st_Heart_Timer.CentrHeartTimer,4);
		TimerDly1+=TimerDly;
		CN_st_Fun_AscNumber.MoveByte(&ptst_Data_Gprs_TcpUdp[j]->CN_st_Heart_Timer.CentrHeartTimer,&TimerDly1,4);
	...
	if(CN_st_GprsHard_UDPDataFlag.NewDataNumber==0)
		return De_Suc_Gprs;
    while(CN_st_GprsHard_UDPDataFlag.NewDataNumber!=0)
	{	
		if((Result=CN_st_Fun_GprsHard.Read(De_GprsHard_Com_SeeDataRead,NULL,NULL,&CN_st_GprsHard_UDPDataFlag,NULL))!=De_Suc_GprsHard)
			return Result;
		if(CN_st_GprsHard_UDPDataFlag.NewDataNumber==0)
...
{	st_GprsHard_CallState	CN_st_GprsHard_CallState;
	...
	{	ptst_Data_Gprs_Call->Mod=CN_st_GprsHard_CallState.CallType;
		ptst_Data_Gprs_Call->PhoneState=CN_st_GprsHard_CallState.State;
		if(CN_st_Affair_Gprs.CN_st_Affair_Phone.ptFunMostPhone!=NULL)			//如果为空
		{	OSSemPost(GprsUseSem);
			CN_st_Affair_Gprs.CN_st_Affair_Phone.ptFunMostPhone(ptst_Data_Gprs_Call);	
			OSSemPend(GprsUseSem, 0, &err);
		}
		return De_Suc_Gprs;
	}
...
{ 	
	...
	{	if(ptst_Data_Gprs_TcpUdp[i]->OpenOrClose!=De_GprsHard_Link_Open)
		...
		if(LinkState!=De_GprsHard_Link_Open)
		{	ptst_Data_Gprs_TcpUdp[i]->OpenOrClose=De_GprsHard_Link_Close;
			if(CN_st_Affair_Gprs.CN_st_Affair_UDPTCP[i].ptFunLinkClose!=NULL)
			{	OSSemPost(GprsUseSem);
				CN_st_Affair_Gprs.CN_st_Affair_UDPTCP[i].ptFunLinkClose(&i);			//中心收不到心跳
				OSSemPend(GprsUseSem, 0, &LinkState);
			}
			return De_Suc_Gprs;
...
{	uint8 State[3];
	uint16 Result;
	uint8 MessageState,PhoneState,IPState,i;
	...
	Result=BackUpSys2();
	CN_st_Affair_Gprs.MessageState=MessageState;
	CN_st_Affair_Gprs.PhoneState=PhoneState;
	CN_st_Affair_Gprs.IPstate=IPState;
	for(i=0;i<De_GprsHard_MaxLink;i++)
		ptst_Data_Gprs_TcpUdp[i]->OpenOrClose=State[i];
	return Result;
}
static uint16 BackUpAPNAndIp(void)
{ 	uint8 State[3];
	uint16 Result;
	uint8 MessageState,PhoneState,IPState,i;
	MessageState=CN_st_Affair_Gprs.MessageState;
...
static uint16 BackUpSys2(void)											//恢复系统状态
{	uint8 State[3];
	uint16 Result;
