...
{	uint16 i,j;
	...
	fd = fd;	 
	if(CN_FtpDown_Data.state == De_FTP_Operation_Work)   //FTP下载状态
	{	CN_st_Fun_DownFile.ReadFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0,0,0);
		for(i=DataLength,j=0;i>0;) 
		{	if((CN_st_Buf_rwFlash.DataLength+i)>=512)
			{	CN_st_Fun_AscNumber.MoveByte(&CN_st_Buf_rwFlash.Buf[CN_st_Buf_rwFlash.DataLength],&Data[j],512-CN_st_Buf_rwFlash.DataLength);		
				CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,CN_st_Buf_rwFlash.Buf,512);
				CN_st_UpFileInfor.RevFileLen +=512; 
				CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0);
				i-=512-CN_st_Buf_rwFlash.DataLength;
				j=DataLength-i;
				CN_st_Buf_rwFlash.DataLength=0;
			}
			else
			{	CN_st_Fun_AscNumber.MoveByte(&CN_st_Buf_rwFlash.Buf[CN_st_Buf_rwFlash.DataLength],&Data[j],i);
				CN_st_Buf_rwFlash.DataLength+=i;
				if((CN_st_UpFileInfor.RevFileLen+i)==CN_st_UpFileInfor.TotalFileLen)
				{	CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,CN_st_Buf_rwFlash.Buf,512);	
					CN_st_UpFileInfor.RevFileLen=CN_st_UpFileInfor.TotalFileLen;
					CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0);
				}
				i=0;	
			}
...
{ 	uint16 result;	 
	result = 1;
	if(CN_st_UpFileInfor.RevFileLen == CN_st_UpFileInfor.TotalFileLen)
	{	
		...
		CN_st_Fun_DownFile.ReadFileTail(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor.TailRecoderNum);
	  	CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0);		
		result =  De_Suc_FTP;	 //下载完成
	}
	else if(CN_st_UpFileInfor.RevFileLen > CN_st_UpFileInfor.TotalFileLen) //下载的文件长度错误
	{	CN_st_UpFileInfor.UpFlag = 0;
		CN_st_UpFileInfor.RevFileLen = 0;
		CN_st_UpFileInfor.TotalFileLen = 0;
		CN_st_Fun_DownFile.ReadFileTail(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor.HeadRecoderNum);
	  	CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0);
...
{
	uint16 result;
	result=CN_st_Fun_DownFile.Open("UpData",&CN_FTP_ID.FtpDownFd);
	if(result == De_Erro_File_NoFile)
	{	CN_st_Fun_DownFile.CreateFile("UpData",De_Max_FileSize/512+1,sizeof(CN_st_UpFileInfor));
		result=CN_st_Fun_DownFile.Open("UpData",&CN_FTP_ID.FtpDownFd);	
	}
	return result;
}
static void InitDownload(st_FTP_Data *pFtpData)
{
	...
	result = CN_st_Fun_DownFile.ReadFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0,0,0);	  //读取状态
	result1 = CN_st_Fun_AscNumber.CompareBuf( CN_st_UpFileInfor.FileName,        		\
											 CN_FtpDown_Data.cn_FTP_Data.SaveFileName ,   		\
											 sizeof(CN_st_UpFileInfor.FileName));
	...
	{					
		...
		CN_st_Fun_AscNumber.MoveByte(CN_st_UpFileInfor.FileName,            \
									 CN_FtpDown_Data.cn_FTP_Data.SaveFileName,        \
        ...
        CN_st_UpFileInfor.TotalFileLen = 0;
		CN_st_Fun_DownFile.ReadFileTail(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor.HeadRecoderNum);
	  	CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0); //将FTP参数和状态写入记录   		
	}
...
static uint16 SendCtrCmd(uint32 StartDownAddr,uint32 *FileLength)
{
    uint16 result;
    uint8  buf[20];	 	
    ...
    {  				
        ...
        FTP_SendCommand(pType_CMD,"I","200");	  	  // 设置文件类型 			
        if(result != De_Suc_FTP) break;	  
        ...
        CN_st_Fun_AscNumber.Algorism2Asc(StartDownAddr,buf);	
        ...
        GetFileLength(FTP_Cmd.RevAsk.askmsg,FileLength); 
		break;
    }
    return result;    
}

static void QuitFtp(void)
{
	...
	CN_st_Fun_DownFile.Close(CN_FTP_ID.FtpDownFd);					
...
{ 
	...
	{ 	
		...
		{	CN_st_Fun_Uart.Write(0,"Datalink timeout\r\n",CN_st_Fun_AscNumber.Strsize("Datalink timeout\r\n")); 						
			QuitFtp();  
			pFtpBox->Result = De_Erro_FTP_TimeOut;				
		...
		{ 	
			...
			CN_st_Fun_DownFile.ReadFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0,0,0);
            result = SendCtrCmd(CN_st_UpFileInfor.RevFileLen,&CN_st_UpFileInfor.TotalFileLen);	
            if(result == De_Suc_FTP)
			{	CN_st_Fun_DownFile.WriteFile(CN_FTP_ID.FtpDownFd,&CN_st_UpFileInfor,0);
				if(CN_st_UpFileInfor.TotalFileLen > CN_st_UpFileInfor.RevFileLen) 
					continue;   	 					
			}
		}
		if(FtpDownFlag == De_FTP_Down_Nofinish)
...
{
    ptst_FTP_WorkState->FileLen       = CN_st_UpFileInfor.TotalFileLen;   // 文件长度
    ptst_FTP_WorkState->DownLoaderLen = CN_st_UpFileInfor.RevFileLen;  // 已经下载的文件长度
