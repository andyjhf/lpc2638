...
static uint16 Explain(uint8 *Buf,void *OutBuf,uint16 DataLen,uint16 ID)
 {	uint16 i;
	for(i=0;i<sizeof(CN_Ba_st_Tz_Agreen_FunOper)/sizeof(st_Tz_Agreen_FunOper);i++,WatchDog())
...
{	uint8 Asc[20];
	...
	CN_st_Fun_AscNumber.ClsArray(ptst_AgreeOn_UpdateSoftWare,sizeof(st_AgreeOn_UpdateSoftWare));
	Len=Buf[1];
	if(Len>=sizeof(Asc))
		Len=sizeof(Asc)-1;
...
{	uint16 i,WorkID,Len;
	...
	CN_st_Fun_AscNumber.ClsArray(ptst_AgreeOn_SetVidType,sizeof(st_AgreeOn_SetVidType));
...
{	st_AgreeOn_TabelList	*ptst_AgreeOn_TabelList;
	...
	CN_st_Fun_AscNumber.ClsArray(ptst_AgreeOn_TabelList,sizeof(st_AgreeOn_TabelList));
...
{	st_AgreeOn_Agree_Track	*ptst_AgreeOn_WorkParSpace;
	ptst_AgreeOn_WorkParSpace=OutBuf;
   	ptst_AgreeOn_WorkParSpace->ComMod=Buf[0];
...
{	st_AgreeOn_Driver	*ptst_AgreeOn_Driver;
	...
	CN_st_Fun_AscNumber.ClsArray(ptst_AgreeOn_Driver,sizeof(st_AgreeOn_Driver));
...
{	st_AgreeOn_HandAttemper	*ptst_AgreeOn_HandAttemper;
	...
	CN_st_Fun_AscNumber.ClsArray(ptst_AgreeOn_HandAttemper,sizeof(st_AgreeOn_HandAttemper));
