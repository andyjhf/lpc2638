...
{	st_NoFlow_Table	CN_st_NoFlow_Table;	
	...
	{	Cuit=CN_st_Fun_File.GetPointNum(CN_st_File_fd.Reducefd,CN_st_NoFlow_Table.AcceSpace)-1;
...
{	
	...
	{	if((ptst_WorkFlow_Table->CN_st_WorkVal_Tabel.Type[Tail]==De_st_AgreeOn_Send_Waring_Type_Yes)
		&&(un.CN_st_File_WorkPar.CN_st_File_WorkParVal[Tail].YesOnON==De_WordPar_Open))
			continue;
		/*if(un.CN_st_File_WorkPar.CN_st_File_WorkParVal[Tail].YesOnON==De_WordPar_Open)
		{	ptst_WorkFlow_Table->CN_st_WorkVal_Tabel.Type[Tail]=De_st_AgreeOn_Send_Waring_Type_Yes;
			continue;
		} */
		ptst_WorkFlow_Table->CN_st_WorkVal_Tabel.Type[Tail]=De_st_AgreeOn_Send_Waring_Type_No;
	}
	...
	{	un.CN_st_NoFlow_Table.ID=ID;
		un.CN_st_NoFlow_Table.AcceSpace=Tail;
		CN_st_Fun_File.WriteRecoder(CN_st_File_fd.NoFlowfd,0,&un.CN_st_NoFlow_Table);
...
{	st_Reduce_Table	CN_st_Reduce_Table;
	...
	if(CN_st_Reduce_Table.Number<1)
		return De_Erro_NoSend;
	Cuit=CN_st_Fun_File.GetPointNum(CN_st_File_fd.Workfd,CN_st_Reduce_Table.Data[CN_st_Reduce_Table.Number-1])-1;
...
{	st_Reduce_Table	CN_st_Reduce_Table;
	...
	if(((DataLen+17)+DataLen/CN_st_Reduce_Table.Number)>486)
		return De_Suc_AccessBasic; 
	return De_Erro_NoSend;	
}
