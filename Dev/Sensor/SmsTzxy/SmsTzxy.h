/*==========================================Include Define=====================================================*/	
	#include "..\..\..\APP\config.h"
	#include "stdlib.h"
	#ifdef De_SmsTzxy_Compiler
		#include "..\..\..\miscellaneous\CodeTrans\AscNumber.h"
		#include "..\..\..\miscellaneous\Date\date.h"
		#include "..\..\..\Control\Gprs\GPRS.h"
		#include "..\..\..\File\rwFile.h"
		#include "..\_PuSensor.h"
	#endif
/*==========================================Define Data========================================================*/
	#define De_Erro_SmsTzxy_Init									1
	#define De_Erro_SmsTzxy_OverMaxNumber							De_Erro_SmsTzxy_Init+1
	#define De_Erro_AgreeOn_Dev_Sms_NoData							De_Erro_SmsTzxy_OverMaxNumber+1
	#define De_Erro_AgreeOn_Dev_Sms_MuchBag							De_Erro_AgreeOn_Dev_Sms_NoData+1
	#define De_Erro_AgreeOn_Dev_Sms_Send							De_Erro_AgreeOn_Dev_Sms_MuchBag+1
	/*+++++++++++++++++++++++++++++++++++++HardWare Define+++++++++++++++++++++++++++++++++++++++++++++++*/
#ifdef De_SmsTzxy_Compiler	
	#define De_GprsTzxy_Fun_Open									1
	#define De_GprsTzxy_Fun_Close									2
	#define De_GprsTzxy_Dev_Number									2
	#define De_GprsTzxy_Nofd										0xff
	#define De_Sms_MaxBagNum										8
#endif
	typedef struct
	{	uint16 OrderNumber;							//命令序号
		uint16 DataLen;
		uint8 BagNumber;							//已接收到包数
		uint8 BagSum;								//总包数
		uint8 Phone[15];							//命令来源号码
	}st_Sms_Order_Data;
	typedef struct
	{	uint8 Data[70];
		uint8 Len;
	}st_Sms_Data;
/*==========================================Static Function====================================================*/
#ifdef De_SmsTzxy_Compiler	
	static uint16 Init(void);
	static uint16 Open(st_Sensor_DataPar	*ptst_Sensor_DataPar,uint8 *fd);
	static uint16 Close(uint8 fd);
	static uint16 Read(uint8 fd,void *SendBuf,void *AgreeBuf);
	static uint16 Send(uint8 fd,uint8 *Buf,uint16 DataLen);
	static uint16 Agree(uint8 fd,uint8 *Buf,uint16 *ReLen);
#endif
/*==========================================Public Ram=========================================================*/
#ifdef De_SmsTzxy_Compiler	
	static OS_EVENT	*SmsSem=NULL;
	st_Sms_Order_Data CN_st_Sms_Order_Data={0xffff,0x0000,0x00,0x00,{0}};
	st_Sms_Data CN_st_Sms_Data[De_Sms_MaxBagNum];
	uint8 AgreeBuf[160];
	uint8 Phone[15];
	uint16 SendOrderNumber=0;	
	static uint8 CN_State[De_GprsTzxy_Dev_Number];
	static uint8 CN_Gprsfd[De_GprsTzxy_Dev_Number];	
	static const st_Fun_Sensor2Hard	CN_st_Fun_Sensor2Hard={Init,Open,Close,Read,NULL};
	static const st_Sensor_Par2Type	CN_st_Sensor_Par2Type[]=
	{	{	De_Sensor_WordPar_MessagTzxy,De_Sensor_Par_Tzxy}
	};
	const st_Sensor_ParList		CN_SmsTzxy_st_Sensor_ParList=
	{	&CN_st_Fun_Sensor2Hard,CN_st_Sensor_Par2Type,sizeof(CN_st_Sensor_Par2Type)/sizeof(st_Sensor_Par2Type)
	};
#else
  	extern st_Sensor_ParList	CN_SmsTzxy_st_Sensor_ParList;	
#endif


