...
{	uint8 i;
	...
	if((SensorSem=OSSemCreate(1))==NULL)
		return De_Erro_Senor_Init;
...
{	uint16 Result;
	...
	if(CN_st_Sensor_Oper[fd].ptst_Fun_Les->GetState!=NULL)
	{	if((Result=CN_st_Sensor_Oper[fd].ptst_Fun_Les->GetState(ptst_Sensor_State,CN_st_Sensor_Oper[fd].fd))!=De_Suc_Sensor)
		{	OSSemPost(SensorSem);
			return Result;
		}
	}
