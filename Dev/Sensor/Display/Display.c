...
{	union
	...
	{	AgreeBuf[DataLen++]=(*CN_st_VaTran.Vaint16>>8);
		AgreeBuf[DataLen++]=(*CN_st_VaTran.Vaint16&0xff);
	}
	...
	{	AgreeBuf[DataLen++]=(*CN_st_VaTran.Vauint16>>8);
		AgreeBuf[DataLen++]=(*CN_st_VaTran.Vauint16&0xff);
	}
	else if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_uint8)
	{	AgreeBuf[DataLen++]=*CN_st_VaTran.Vauint8;	
	}
	else if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_uint32)
	{	AgreeBuf[DataLen++]=((*CN_st_VaTran.Vauint32)>>24);
		AgreeBuf[DataLen++]=((*CN_st_VaTran.Vauint32)>>16);
		
		AgreeBuf[DataLen++]=((*CN_st_VaTran.Vauint32)>>8);
		
		AgreeBuf[DataLen++]=((*CN_st_VaTran.Vauint32)&0xff);
	}
	else if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_MacID)
	{	HexByte=CN_st_Fun_AscNumber.BCD2Algorism(CN_st_VaTran.Vauint8,5);
		AgreeBuf[DataLen++]=0;
		AgreeBuf[DataLen++]=((HexByte)>>24);
		AgreeBuf[DataLen++]=((HexByte)>>16);
		
		AgreeBuf[DataLen++]=((HexByte)>>8);
		
		AgreeBuf[DataLen++]=((HexByte)&0xff);
...
{	union
	...
	if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_int16)
		*CN_st_VaTran.Vauint16=Buf[0]*0x100+Buf[1];
	else if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_Asc)
	{	if(SizeofAgreeBuf>CN_st_Display_Par_Set[i].SizeofPoint)
			SizeofAgreeBuf=CN_st_Display_Par_Set[i].SizeofPoint;
		CN_st_Fun_AscNumber.MoveByte(CN_st_VaTran.Vauint8,Buf,SizeofAgreeBuf);	
	}
	else if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_IP)
	{	CN_st_Fun_AscNumber.Char2IP(Buf,CN_st_VaTran.Vauint32,'.');
		...
		*CN_st_VaTran.Vauint8=Buf[0];	
	else if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_uint32)
		*CN_st_VaTran.Vauint32=Buf[0]*0x1000000+Buf[1]*0x10000+Buf[2]*0x100+Buf[3];
	else if(CN_Ba_st_Display_Par2Type[i].Type==De_Display_Par_Type_MacID)
	{	Hext=Buf[1]*0x1000000+Buf[2]*0x10000+Buf[3]*0x100+Buf[4];
		CN_st_Fun_AscNumber.Algorism2BCD(Hext,CN_st_VaTran.Vauint8,5);	
	}
...
{	st_Display_Agree	*ptst_Display_Agree;
	...
	{	CN_st_Un_Explain.ptst_Display_Agree_Phone->Type=X_BUF[4];
		...
		if(DataLen>20)
			DataLen=20;
		CN_st_Fun_AscNumber.MoveByte(CN_st_Un_Explain.ptst_Display_Agree_Phone->Phone,X_BUF+5,DataLen-6);
	}
	...
	{	CN_st_Un_Explain.ptst_Display_Agree_SeePar->PassWord=X_BUF[4]*0x1000000+X_BUF[5]*0x10000+X_BUF[6]*0x100+X_BUF[7];
		...
		if(CN_st_Un_Explain.ptst_Display_Agree_SeePar->PassWord!=CN_st_File_GreenPar.PassWord)
			return De_Erro_Display_Explain;
		Result=DataLen-9;
		Result=Result/2;
		CN_st_Fun_AscNumber.ClsArray(CN_st_Un_Explain.ptst_Display_Agree_SeePar->YesOnNo,
		sizeof(CN_st_Un_Explain.ptst_Display_Agree_SeePar->YesOnNo));
		for(i=0;i<Result;i++)
		{	Par=X_BUF[8+i*2]*0x100+X_BUF[8+i*2+1];
			for(DataLen=0;DataLen<sizeof(CN_Ba_st_Display_Par2Type)/sizeof(st_Display_Par2Type);DataLen++)
			{	if(Par==CN_Ba_st_Display_Par2Type[DataLen].Par)
					break;
			}
			if(DataLen==sizeof(CN_Ba_st_Display_Par2Type)/sizeof(st_Display_Par2Type))
				continue;
			CN_st_Un_Explain.ptst_Display_Agree_SeePar->YesOnNo[DataLen]=1;
		}	
	}
	else if(X_BUF[3]==De_Display_Device_Order_SetPar)			//设置参数命令
	{	CN_st_Fun_AscNumber.ClsArray(CN_st_Un_Explain.ptst_Display_Par,sizeof(st_Display_Par));
		//CN_st_Fun_AscNumber.ClsArray(CN_st_Un_Explain.ptst_Display_Par->Set,sizeof(CN_st_Un_Explain.ptst_Display_Par->Set));
		CN_st_Un_Explain.ptst_Display_Par->PassWord=X_BUF[4]*0x1000000+X_BUF[5]*0x10000+X_BUF[6]*0x100+X_BUF[7];
	  	Par=X_BUF[8]*0x100+X_BUF[9];
	   	CommType2App(X_BUF+10,CN_st_Un_Explain.ptst_Display_Par,Par,DataLen-11);
	}
