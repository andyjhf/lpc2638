...
{	st_Display_Agree	*ptst_Display_Agree;
	...
	CN_st_Fun_Timer.DisEnable(DisTimerfd);
	for(i=0;i<sizeof(CN_Ba_st_DispLay_Oper_Fun)/sizeof(st_DispLay_Oper_Fun);i++,WatchDog())
...
{	union
	...
	CN_st_Display_Send.ID=De_Display_Host_Order_ParAnswer;
	CN_st_Display_Send.SendData=&CN_st_Display_Par;
	for(i=0;i<sizeof(((st_Display_Agree_SeePar	*)(NULL))->YesOnNo);i++)
	{	CN_st_Display_Par.Set[i]=0;
		if(ptst_Display_Agree_SeePar->YesOnNo[i]==0)
			continue;
...
{	union
	...
	{	
		...
		CN_st_VaTran.Buf=(void *)((uint32 )&ptst_Display_Par->CN_st_Display_Par_Cut+
			(uint32 )CN_st_Display_Par_Set[i].Point);				
		CN_st_VaTran1.Buf=(void *)((uint32 )&un.CN_st_Parameter_Table+
		(uint32 )CN_Ba_st_SeePar_Data[i].Dev);
		CN_st_Fun_File.ReadRecoder(*CN_Ba_st_SeePar_Data[i].fd,0,&un.CN_st_Parameter_Table);
		if(CN_st_Display_Par_Set[i].Type==De_Display_Par_Type_Asc)
			CN_st_Fun_AscNumber.MoveByte(CN_st_VaTran1.Buf,CN_st_VaTran.Buf,CN_st_Display_Par_Set[i].SizeofPoint);
		else if((CN_st_Display_Par_Set[i].Type==De_Display_Par_Type_IP)||(CN_st_Display_Par_Set[i].Type==De_Display_Par_Type_uint32))
			*CN_st_VaTran1.Vauint32=*CN_st_VaTran.Vauint32;
		else if(CN_st_Display_Par_Set[i].Type==De_Display_Par_Type_uint8)
			*CN_st_VaTran1.Vauint8=*CN_st_VaTran.Vauint8;
		else if(CN_st_Display_Par_Set[i].Type==De_Display_Par_Type_uint16)
			*CN_st_VaTran1.Vauint16=*CN_st_VaTran.Vauint16;
		else if(CN_st_Display_Par_Set[i].Type==De_Display_Par_Type_int16)
			*CN_st_VaTran1.Vaint16=*CN_st_VaTran.Vaint16;
	...
	{	
		...
		if(CN_Ba_st_SeePar_Data[i].fd==NULL) 
			continue;
		for(j=0;j<(sizeof(CN_Ba_Log)/sizeof(uint16));j++)
		{	if(CN_Ba_Log[j]==CN_st_Display_Par_Set[i].Par)
...
{	st_Display_Send		CN_st_Display_Send;
	...
	{ 	CN_st_Fun_File.ReadRecoder(CN_st_File_fd.Rootfd,i+1,&CN_st_AgreeOn_Phone_Par);
		if(CN_st_AgreeOn_Phone_Par.State==0xff)
			continue;
