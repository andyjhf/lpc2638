...
{	uint8 i;
	...
	{	if(*CN_Ba_TimerPar_Term[i].Val1==De_No_Data_Send)
		...
		*CN_Ba_TimerPar_Term[i].Val2=0;
	}		  
}

static uint32 GetTimeAdd(uint8 *Small,uint8 *Big)
{	uint8 Curtimer[7],ByteV;
	uint32 DateNumber;
	uint32 BigNumber,SmallNumber;
	...
	for(DateNumber=0;CN_st_Fun_AscNumber.CompareBuf(Curtimer,Big,4)!=0;DateNumber++)
		CN_st_Fun_Date.Next(Curtimer,De_Type_Date_Date);
	CN_st_Fun_AscNumber.Bcd2Hex(Big+4,&ByteV,1);
	BigNumber=DateNumber*86400+ByteV*3600;
	CN_st_Fun_AscNumber.Bcd2Hex(Big+5,&ByteV,1);
	BigNumber=BigNumber+ByteV*60;
	CN_st_Fun_AscNumber.Bcd2Hex(Big+6,&ByteV,1);
	BigNumber=BigNumber+ByteV;

	CN_st_Fun_AscNumber.Bcd2Hex(Small+4,&ByteV,1);
	SmallNumber=ByteV*3600;
	CN_st_Fun_AscNumber.Bcd2Hex(Small+5,&ByteV,1);
	SmallNumber=SmallNumber+ByteV*60;
	CN_st_Fun_AscNumber.Bcd2Hex(Small+6,&ByteV,1);
	SmallNumber=SmallNumber+ByteV;	
...
{	st_Parameter_Table	CN_st_Parameter_Table;
	...
	{	CN_st_Fun_AscNumber.MoveByte(DateTime,NonceTimer,4);
		...
		if(CN_st_Fun_AscNumber.CompareVa(DateTime,NonceTimer,7,De_AscNumber_Va_Big)==De_AscNumber_Va_Flase)
			...
			EndTimerNumber[i]=GetTimeAdd(NonceTimer,DateTime);	
		*CN_Ba_TimerPar_Term[i-8].Val2=0;		
	}				
}
/*=========================================ACC时间类============================================================*/
