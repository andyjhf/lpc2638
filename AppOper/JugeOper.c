...
{	uint8 i;
	JugeAppair(Number,CN_st_File_fd.WaringTermfd,De_Access_Space_WorkParWaring1,NULL,De_Access_Type_WorkParWaring);
	...
	if(i!=De_CN_Ba_WordPar_Number)
		App_CN_st_WorkFlow_Table.CN_st_AgreeOn_Send_Basic.State[De_Access_Type_WorkParWaring]=De_AgreeOn_State_Erro;
	else
		App_CN_st_WorkFlow_Table.CN_st_AgreeOn_Send_Basic.State[De_Access_Type_WorkParWaring]=De_AgreeOn_State_Suc;
}
...
{	st_AgreeOn_SetWaringTerm	CN_st_AgreeOn_SetWaringTerm;
	uint8 State;
	if(CN_st_Fun_File.ReadRecoder(fd,0,&CN_st_AgreeOn_SetWaringTerm)!=De_Suc_File)
		return ;
