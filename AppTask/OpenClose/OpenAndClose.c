...
{	st_Sensor_DataPar	CN_st_Sensor_DataPar;
	uint8 fd;
	CN_st_Fun_Timer.EditPro(Timerfd,120);
...
{	uint8 fd;
	...
	if(CN_st_Fun_Tzv21.GetPar2ID(De_Sensor_WordPar_GprsTzxy_1,&fd)!=De_Suc_AgreeOn)
		return ;
	if(CN_st_Fun_AppSendAgreeOn.WriteCommand(fd,De_Dev_Order_Login,NULL)!=De_Suc_AgreeOn)
	{	LogFlag=De_No_Log;
		OpenGprs();
	...
	LogFlag=De_LogSuc;
	CN_st_Fun_Timer.DisEnable(LogTimerfd);
	CN_st_Fun_AppSendAgreeOn.NoSemSend(); 				//暂时先封闭盲区补偿
