...
{	uint8 err;
	...
	{	ptst_BoxData_UpData = OSQPend(UpDateSoftWare_Q,UpDataWaitTime,&err);
		...
		if(err!=OS_NO_ERR)
		{	CN_st_Fun_FTP.GetState(&CN_st_FTP_WorkState_Now);
			if(DownLoaderState!=NULL)
				DownLoaderState(&CN_st_FTP_WorkState_Now);
		...
		{	if((CN_st_FTP_Data.Result!=0xffff)&&(CN_st_FTP_Data.Result!=De_Suc_FTP))
			...
			{	if(UpDataWaitTime!=0)
				...
				DownLoaderOver=ptst_BoxData_UpData->CN_st_Order_UpdateSoftWare.DownLoaderOver;
				DownLoaderState=ptst_BoxData_UpData->CN_st_Order_UpdateSoftWare.DownLoaderState;	
				UpDataWaitTime=100;							  //下载开始，定时查询下载进度
				ptst_BoxData_UpData_Temp=ptst_BoxData_UpData;
				CN_st_Fun_AscNumber.ClsArray(&CN_st_FTP_Data,sizeof(st_FTP_Data));
				CN_st_FTP_Data.Result=0xffff;
				...
				CN_st_Fun_AscNumber.MoveByte(CN_st_FTP_Data.SaveFileName,ptst_BoxData_UpData->CN_st_Order_UpdateSoftWare.SaveFileName,DataLength);
